...
const Roadmap = () => {
  const { roadmapRoles, loading, error } = useData();

  if (loading) {
    return (
      <div className="h-full md:px-24 flex-col md:flex-row flex w-full mx-auto text-white flex-wrap justify-center items-center">
  ...
  return (
    <div className="h-full md:px-24 flex-col md:flex-row flex w-full mx-auto text-white flex-wrap">
      {roadmapRoles?.map(({ title, url }) => (
        <a key={title} href={url} target="_blank" rel="noopener noreferrer">
          <div className="border-2 border-second-300 shadow-md my-8 shadow-second-350 h-12 w-60 mx-auto md:w-64 md:mx-12 rounded-xl p-2 md:m-4 flex justify-between">
            {title}
            <svg
              width="8"
              height="10"
